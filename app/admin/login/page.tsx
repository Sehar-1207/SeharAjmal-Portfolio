...
export default function AdminLoginPage() {
  ...
  return (
    <div className="w-full max-w-md bg-card border border-border shadow-2xl rounded-2xl p-6 md:p-8 mx-auto">
      ...
      {/* Error Alert */}
      {error && (
        <div className="mb-6 p-3.5 bg-destructive/10 border border-destructive/20 text-destructive text-xs font-bold rounded-xl transition-all">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleLogin} className="space-y-5">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-primary text-primary-foreground text-sm font-bold py-3.5 min-h-[44px] rounded-xl hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center"
        >
